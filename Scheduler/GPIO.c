...
{
	return ACCESS_REG_8BIT((port + uint16PUT_OFFEST));
}
...
bool_t pin_read(uint16 pin)                                                             //READ FROM PIN
{
	...
	return (IS_BIT_SET((port + uint16PUT_OFFEST),pin));
}
...
void set_pins(uint16 pin_start, uint16 pin_end)
{
	port_t port;
	if(pin_start <= END_PORTA)
	{
...
void clear_pins(uint16 pin_start, uint16 pin_end)
{
	port_t port;
	if(pin_start <= END_PORTA)
	{
